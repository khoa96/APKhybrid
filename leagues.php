<?php include "templates/head.php"; ?>

<body>
    <?php include "templates/header.php"; ?>
    <?php include "templates/menu.php"; ?>
    <div class="all-league-page-wrapper">
        <div class="container">
            <div class="all-league-page-content">
                <span class="label-filter">Football</span>

                <h3 class="league-title">Hot League</h3>
                <ul class="list-league">
                    <li class="league-item">
                        <a href="#">EPL - Premier League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Serie A</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Bundesliga</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Championship</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Ligue 1</a>
                    </li>
                    <li class="league-item">
                        <a href="#">MLS</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Serie B</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Liga MX</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Bundesliga</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Chinese Super League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Serie A</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Superliga</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Ligue 2</a>
                    </li>
                    <li class="league-item">
                        <a href="#">2. Bundesliga</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Ligue 2</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Eredivisie</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Primeira Liga</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Premier League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Primera Division</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Segunda Division</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Allsvenskan</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Super League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Super Lig</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Champions League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Champions League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">EPL - Premier League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Serie A</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Bundesliga</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Championship</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Ligue 1</a>
                    </li>
                    <li class="league-item">
                        <a href="#">MLS</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Serie B</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Liga MX</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Bundesliga</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Chinese Super League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Serie A</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Superliga</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Ligue 2</a>
                    </li>
                    <li class="league-item">
                        <a href="#">2. Bundesliga</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Ligue 2</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Eredivisie</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Primeira Liga</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Premier League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Primera Division</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Segunda Division</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Allsvenskan</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Super League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Super Lig</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Champions League</a>
                    </li>
                    <li class="league-item">
                        <a href="#">Champions League</a>
                    </li>

                </ul>

                <div class="box-search-league-wrapper">
                    <div class="search-wrapper-common sidebar-search">
                        <span class="icon icon-search">

                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M16.25 16.25L13.3333 13.3333M15 8.74998C15 11.7415 12.5748 14.1666 9.58329 14.1666C6.59175 14.1666 4.16663 11.7415 4.16663 8.74998C4.16663 5.75844 6.59175 3.33331 9.58329 3.33331C12.5748 3.33331 15 5.75844 15 8.74998Z"
                                    stroke="#1A202C" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>

                        </span>
                        <input type="text" name="" value="" placeholder="Search" />
                    </div>
                </div>

                <div class="box-all-league-wrapper">
                    <div class="header-control">
                        <h3 class="league-title">All leagues</h3>
                        <button class="btn-toggle-collapse">
                            <span class="label">Collapse all</span>
                            <span class="icon-collapse">
                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <g clip-path="url(#clip0_1652_16623)">
                                        <path d="M6 4L10 8L6 12" stroke="#111111" stroke-width="1.5"
                                            stroke-linecap="round" stroke-linejoin="round"></path>
                                    </g>
                                    <defs>
                                        <clipPath id="clip0_1652_16623">
                                            <rect width="16" height="16" fill="white"></rect>
                                        </clipPath>
                                    </defs>
                                </svg>
                            </span>
                        </button>
                    </div>
                    <ul class="list-country-groups">
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                        <li class="country-group">
                            <div class="country-group-title">
                                <img src="https://flagcdn.com/w320/vn.png" class="flag-country" alt="" />
                                <strong>Albania</strong>
                            </div>
                            <ul class="list-countries">
                                <li>
                                    <a href="#" class="">Ligue 1</a>
                                </li>
                                <li>
                                    <a href="#" class="">Ligue 2</a>
                                </li>
                                <li>
                                    <a href="#" class="">Algeria Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="" class="">Algeria Youth League</a>
                                </li>
                                <li>
                                    <a href="#" class="">Superliga</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">2nd Division</a>
                                </li>
                                <li>
                                    <a href="#" class="">Albanian Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">1st Division Play-offs</a>
                                </li>
                                <li>
                                    <a href="#" class="">Super Cup</a>
                                </li>
                                <li>
                                    <a href="#" class="">Kampionati i Femrave</a>
                                </li>
                            </ul>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
    <?php include "templates/footer.php"; ?>
    <script>
    $(document).ready(function() {
        $(".btn-toggle-collapse").on('click', function() {
            $(".list-countries").toggle()
            $(this).toggleClass('collapse')

            if ($(this).hasClass('collapse')) {
                $('.btn-toggle-collapse .label').text('Expand all')
            } else {
                $('.btn-toggle-collapse .label').text('Collapse all')
            }

        })
    })
    </script>

</body>


</html>